...
gsap.to(theCube.position, { duration: 1, delay: 1, x: 3 });
gsap.to(theCube.position, { duration: 1, delay: 2, x: 0 });

const clock = new THREE.Clock();

const tick = () => {
  // const timePassed = clock.getElapsedTime();
  // theCube.position.y = Math.cos(timePassed);
  // theCube.position.x = Math.sin(timePassed);
